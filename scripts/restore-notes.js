...
// CSV data with notes for all ingredients
  ...
  try {
    ...
    rows.forEach(row => {
      const pattern = new RegExp(
        `('Ingredient Name': '${row.ingredientName}'[\\s\\S]*?'Substitute Name': '${row.substituteName}'[\\s\\S]*?)Notes: ['"].*?['"]`,
        'g'
      );
      
      const newContent = content.replace(pattern, (match, prefix) => {
        modified = true;
        return `${prefix}Notes: '${row.notes}'`;
      });

      if (newContent !== content) {
        content = newContent;
        console.log(`Updated notes for ${row.ingredientName} -> ${row.substituteName}: "${row.notes}"`);
      }
    });
...
console.log('Finished updating notes in all ingredient files.');